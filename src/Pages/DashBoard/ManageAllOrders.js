...
const ManageAllOrders = () => {
  ...
  return (
    <>
      <h1 className="text-center mt-5 text-2xl font-semibold text-red-700">
        Manage All Orders
      </h1>
      <div className="text-center my-10">
        <button
          className="btn btn-warning btn-sm mr-4"
          onClick={() => setQuery("delivery=pending")}
        >
          Pending orders
        </button>
        <button
          className="btn btn-success btn-sm mr-4"
          onClick={() => setQuery("delivery=shipped")}
        >
          Shipped orders
        </button>
        <button
          className="btn btn-error btn-sm mr-4"
          onClick={() => setQuery("status=unpaid")}
        >
          Unpaid orders
        </button>
        <button className="btn btn-accent btn-sm " onClick={() => setQuery("")}>
          All orders
        </button>
      </div>
      <div className="mr-auto lg:mr-0 ml-auto w-[95%] px-5 mt-5">
        <div className="overflow-x-auto">
          {reversedOrders?.length === 0 ? (
            <h1 className="text-xl text-red-700 text-center font-semibold">
              No order found
            </h1>
          ) : (
            <table className="table w-full">
              ...
              <tbody>
                {reversedOrders?.map((order, index) => (
                  <SIngleManageOrder
                    setDeliverModal={setDeliverModal}
                    key={order._id}
                    order={order}
                    index={index}
